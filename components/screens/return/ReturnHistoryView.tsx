// File: components/features/return/ReturnHistoryView.tsx
...
import type { ReturnRequest, ReturnStatus } from "@/data/mockData";
import ReturnRequestCard from "./ReturnRequestCard"; // <-- Dùng Card mới
import IconButton from "@/components/common/IconButton";

type ReturnHistoryViewProps = {
  requests: ReturnRequest[];
  statusFilters: { label: string; value: ReturnStatus | null }[];
  selectedStatus: ReturnStatus | null;
  selectedDate: Date | null;
  showDatePicker: boolean;
  onStatusChange: (status: ReturnStatus | null) => void;
...
const ReturnHistoryView: React.FC<ReturnHistoryViewProps> = ({
  requests,
  statusFilters,
  selectedStatus,
  selectedDate,
  ...
  return (
    <View className="flex-1 bg-BACKGROUND">
      ...
      <FlatList
        data={requests}
        renderItem={({ item }) => <ReturnRequestCard request={item} />}
