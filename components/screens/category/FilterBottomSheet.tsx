...
// Sửa Interface props để nhận IProductCard
interface FilterBottomSheetProps {
  ...
  allProducts: IProductCard[]; // Dùng IProductCard
}
...
export default function FilterBottomSheet({
  ...
  const { absoluteMin, absoluteMax } = useMemo(() => {
    ...
    // IProductCard dùng field 'price'. Nếu có salePrice thì cần logic khác,
    // ở đây tạm lấy price gốc.
      ...
      >
        ...
        <View className="bg-white rounded-t-2xl pb-5 shadow-lg">
          ...
          <View className="p-5">
            ...
            {/* Ẩn text gợi ý giá nếu không có product truyền vào */}
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-1">
                ...
                <TextInput
                  className="bg-gray-100 rounded-lg py-2.5 px-3 text-sm"
                  placeholder="Từ"
          ...
          <View className="flex-row justify-between px-5 pt-4 border-t border-gray-100">
            <TouchableOpacity
              className="flex-1 py-3 rounded-lg border border-gray-300 items-center mr-2"
              onPress={handleReset}
            >
              <Text className="text-gray-700 font-semibold">Bỏ lọc</Text>
            </TouchableOpacity>
            <TouchableOpacity
              className="flex-1 py-3 rounded-lg bg-green-600 items-center ml-2"
              onPress={handleApply}
            >
